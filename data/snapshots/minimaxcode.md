<!-- https://agent.minimaxi.com/docs/code/account/usage fetched 2026-09-07T15:54:40.734Z -->
Title: Token Plan 与积分 - MiniMax Agent 文档

URL Source: https://agent.minimaxi.com/docs/code/account/usage

Markdown Content:
> ## Documentation Index
> 
> 
> Fetch the complete documentation index at:[/docs/llms.txt](https://agent.minimaxi.com/docs/llms.txt)
> 
> 
> Use this file to discover all available pages before exploring further.

[跳转到主要内容](https://agent.minimaxi.com/docs/code/account/usage#content-area)

[MiniMax Agent 文档 home page![Image 1: light logo](https://mintcdn.com/agent-cn/adVqF7AWWrY2xO08/logo/light.svg?fit=max&auto=format&n=adVqF7AWWrY2xO08&q=85&s=ffdd97a225809e62b5b2c219604e4e54)![Image 2: dark logo](https://mintcdn.com/agent-cn/adVqF7AWWrY2xO08/logo/dark.svg?fit=max&auto=format&n=adVqF7AWWrY2xO08&q=85&s=7c367a39f2a521068e163f5557ff7820)](https://agent.minimaxi.com/)

搜索...

Ctrl K

*   [开始使用](https://agent.minimaxi.com/)

搜索...

导航

模型与账户

Token Plan 与积分

[MiniMax Code](https://agent.minimaxi.com/docs/code/welcome)[CLI](https://agent.minimaxi.com/docs/cli/quick-start)[更新日志](https://agent.minimaxi.com/docs/changelog)[技术博客](https://agent.minimaxi.com/docs/techblog)

### 开始使用

*   [欢迎使用](https://agent.minimaxi.com/docs/code/welcome)
*   [下载与安装](https://agent.minimaxi.com/docs/code/get-started/download)
*   [创建第一个任务](https://agent.minimaxi.com/docs/code/get-started/first-task)

### 核心工作流

*   [Coding 与 Work 模式](https://agent.minimaxi.com/docs/code/workflows/modes)
*   [工作区与项目上下文](https://agent.minimaxi.com/docs/code/workflows/workspace)
*   [任务与历史管理](https://agent.minimaxi.com/docs/code/workflows/tasks)
*   [对话协作](https://agent.minimaxi.com/docs/code/workflows/conversation-collaboration)
*   [代码审查](https://agent.minimaxi.com/docs/code/workflows/code-review)
*   [权限与安全确认](https://agent.minimaxi.com/docs/code/workflows/permissions)

### 桌面端能力

*   [内置浏览器](https://agent.minimaxi.com/docs/code/desktop/browser)
*   [开发面板](https://agent.minimaxi.com/docs/code/desktop/panels)
*   [快捷键与快捷小窗](https://agent.minimaxi.com/docs/code/desktop/shortcuts)
*   [更新与反馈](https://agent.minimaxi.com/docs/code/desktop/updates-feedback)
*   [目标功能](https://agent.minimaxi.com/docs/code/desktop/goal)

### Agent 能力

*   [Agent Team](https://agent.minimaxi.com/docs/code/agents/team)
*   [自定义 Agent](https://agent.minimaxi.com/docs/code/agents/custom-agents)
*   [插件市场](https://agent.minimaxi.com/docs/code/agents/plugins)
*   [MCP 服务](https://agent.minimaxi.com/docs/code/agents/mcp)
*   [记忆机制](https://agent.minimaxi.com/docs/code/agents/memory)

### 自动化与远程

*   [定时任务](https://agent.minimaxi.com/docs/code/automation/schedules)
*   [远程控制](https://agent.minimaxi.com/docs/code/automation/remote-control)
*   [IM 连接](https://agent.minimaxi.com/docs/code/automation/im)

### 模型与账户

*   [Token Plan 与积分](https://agent.minimaxi.com/docs/code/account/usage)
*   [MiniMax API 密钥](https://agent.minimaxi.com/docs/code/account/minimax-api)
*   [自定义模型 / BYOK](https://agent.minimaxi.com/docs/code/account/byok)

### 帮助

*   [常见问题](https://agent.minimaxi.com/docs/code/help/faq)
*   [问题反馈](https://agent.minimaxi.com/docs/code/help/feedback)

## 在此页面

*   [每日签到](https://agent.minimaxi.com/docs/code/account/usage#%E6%AF%8F%E6%97%A5%E7%AD%BE%E5%88%B0)
*   [可以查看什么](https://agent.minimaxi.com/docs/code/account/usage#%E5%8F%AF%E4%BB%A5%E6%9F%A5%E7%9C%8B%E4%BB%80%E4%B9%88)

模型与账户

# Token Plan 与积分

查看 MiniMax Code 的用量、订阅状态和积分余额。

在设置的用量页面中，你可以查看当前 Token Plan、积分余额、使用额度和发票入口。![Image 3: 用量与模型页面](https://mintcdn.com/agent-cn/BK6oKQ5dsiqXZ-jT/images/code/usage-model.png?fit=max&auto=format&n=BK6oKQ5dsiqXZ-jT&q=85&s=f318eee878ccb075447dbec34683f7e3)
## [​](https://agent.minimaxi.com/docs/code/account/usage#%E6%AF%8F%E6%97%A5%E7%AD%BE%E5%88%B0)

每日签到

打开 MiniMax Code 完成每日签到即可领取积分奖励。
*   每日签到可领取 **400 积分**
*   连续签到第 4 天、第 7 天，单日奖励加码至 **1000 积分**
*   完成一周签到，累计可领取 **4000 积分**
*   签到积分可累计使用，自到账之日起 30 天内有效

积分适用于 MiniMax Code 的各类任务场景，从办公、Coding、深度调研和专业数据库调用，到 PPT、文档、图片、音频及视频等多模态创作，都可以直接使用。
## [​](https://agent.minimaxi.com/docs/code/account/usage#%E5%8F%AF%E4%BB%A5%E6%9F%A5%E7%9C%8B%E4%BB%80%E4%B9%88)

可以查看什么

*   当前订阅状态
*   Token Plan 有效期
*   MiniMax 积分余额
*   用量额度
*   订阅、充值、发票和签到入口

具体计费和额度规则以产品内展示为准。

[IM 连接](https://agent.minimaxi.com/docs/code/automation/im)[MiniMax API 密钥](https://agent.minimaxi.com/docs/code/account/minimax-api)

[discord](https://discord.com/invite/hvvt8hAye6)[x](https://x.com/MiniMax_AI)[linkedin](https://www.linkedin.com/company/minimax-ai)[github](https://github.com/MiniMax-AI)

